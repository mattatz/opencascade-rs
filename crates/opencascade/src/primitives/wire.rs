...
impl Wire {
    ...
    /// Returns an iterator over all edges in the wire, ordered and without duplicates
    pub fn edges(&self) -> WireEdgeIterator {
        let explorer = ffi::BRepTools_WireExplorer_ctor(&self.inner);

        WireEdgeIterator { explorer }
    }
